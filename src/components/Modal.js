...
export default function Modal({ func, comid }) {
    const dispatch = useDispatch();
    const { active } = useSelector((store) => store.modal);


  return (
    <div className={active ? 'absolute top-0 left-0 bg-black/30 h-[100vh] w-full flex justify-center z-[999]' : 'hidden'}>
        <div className='w-[90%] md:w-[30%] bg-white rounded m-auto p-5'>
            ...
            <div className='my-3 flex justify-around'>
                ...
                <button onClick={() => func(comid)} className='bg-softRed text-white px-5 py-2 rounded-md'>YES, DELETE</button>
            </div>
        </div>
    </div>
  )
